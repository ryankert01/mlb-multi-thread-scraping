...
def aggregate_data(header1, data1, header2, data2, shift):
    ...
    header, data = header1 + header2[j+1:], [data1[i] + data2[i][j+1:] for i in range(len(data1))]
    if shift == 0:
        assert len(header) == 35 + 1
    elif shift == 1:
        assert len(header) == 41 + 1
    elif shift == 2:
        assert len(header) == 30 + 1
    elif shift == 3:
        assert len(header) == 37 + 1
    return header, data